...
- **preflight.rs**: Docker startup probe — non-fatal warning when changed files won't resolve in the container (exec: repo-relative path vs container WORKDIR; `docker run` fallback without `volume_mount`)
