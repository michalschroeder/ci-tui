...
Stop pushing to find out CI is red. `ci-tui` detects your changed files against the base branch, figures out which checks and which *tests* are relevant, and runs them in the same Docker containers your CI uses — streaming output live in a TUI.
...
- **CI parity**: checks execute via `docker compose exec` in your project's containers. Green here predicts green in CI.

## Features

- Live TUI (ratatui) with per-check status, streamed output, and timing
...
- Falls back to `docker run` when the compose container isn't up
...
| `X` | Fix all checks |
| `c` | Copy check command to clipboard |
...
Simple mode is auto-enabled when stdout is not a terminal, so `ci-tui` works as a CI runner. This repo dogfoods it: its own [`ci-tui.yaml`](ci-tui.yaml) runs fmt, clippy and tests in the dev image.

## How check selection works

1. `git diff` against the base ref lists changed files (minus `ignore_patterns`).
2. Each check's `triggers.file_pattern` is matched against the list.
3. Checks with `test_discovery` map changed sources to test files; if none are found and the check is `on_demand`, it waits for you to press `t`.
